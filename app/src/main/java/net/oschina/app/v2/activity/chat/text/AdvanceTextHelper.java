 ...
 */
public class AdvanceTextHelper {
    /**
    ...
    public static void parseLinkText(TextView widget, String sourceText) {
        Spannable spp = Spannable.Factory.getInstance().newSpannable(
                Html.fromHtml(sourceText));
        URLSpan[] spans = spp.getSpans(0, spp.length(), URLSpan.class);
        for (URLSpan span : spans) {
            Log.d(TAG, "init span:" + span.getURL());
            int start = spp.getSpanStart(span);
            int end = spp.getSpanEnd(span);
            spp.removeSpan(span);
            spp.setSpan(new MyURLSpan(span.getURL()), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }

        widget.setText(spp);
        widget.setAutoLinkMask(Linkify.WEB_URLS | Linkify.PHONE_NUMBERS | Linkify.EMAIL_ADDRESSES);
        ...
        if (text instanceof Spannable) {
            ...
            for (URLSpan span : urls) {
                ...
                int ss = sp.getSpanStart(span);
                int ee = sp.getSpanEnd(span);
                sp.removeSpan(span);

                Log.d(TAG, "url span:" + span.getURL() + " ss:" + ss + ":" + ee);
                sp.setSpan(new MyURLSpan(span.getURL()), ss, ee, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
